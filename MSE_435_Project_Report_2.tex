...
\subsection*{Policy Validation Design}

To validate the robustness of the generated schedules, we evaluate three operational policies as
post-processing transforms on the same baseline schedule outputs rather than re-solving the
optimization model. This isolates policy effects from solver-path effects and supports direct
like-for-like comparisons.

\begin{itemize}
  \item \textbf{Single-room policy:} each provider-day is mapped to a single anchor room, chosen
  as the modal room in that provider-day's baseline assignments. This deterministic choice avoids
  arbitrary tie-breaking and is a practical proxy for ``one room per provider per day/week.''
  \item \textbf{Room-cluster policy:} each provider-day uses an anchor room and a room cluster
  defined by distance threshold $\le 2.0$ from that anchor. The value $2.0$ is used as a balanced
  middle ground: $\le 1.0$ is often too restrictive, while $\le 3.0$ can become close to
  unconstrained movement.
  \item \textbf{Blocked-days policy (auto-strong):} providers are assigned blocked weekdays in a
  round-robin pattern (Monday to Friday). This is intentionally a stress-test policy, not a
  representation of baseline operations, and is used to quantify sensitivity to broad availability
  restrictions.

\end{itemize}

For comparability with the baseline, room-switch distance and preference penalty are computed
using the same metric definitions as in the main implementation. For blocked days, we additionally
report blocked-appointment ratio as a policy-feasibility impact metric.
...
\subsection*{Validation Across Room and Availability Policies}

Table~\ref{tab:policy_validation} reports policy comparisons computed from the baseline run
(\texttt{outputs/run\_20260320\_173628}) without rerunning column generation.

\begin{table}[h]
  \centering
  \caption{Policy validation on fixed baseline schedules (no re-optimization).}
  \label{tab:policy_validation}
  \begin{tabular}{lrr}
    \toprule
    Metric & Week 1 & Week 2 \\
    \midrule
    Baseline switch distance                  & 339.7 & 600.9 \\
    Single-room switch distance               & 0.0   & 0.0   \\
    Cluster ($\le 2.0$) switch distance       & 160.6 & 245.0 \\
    Blocked-days switch distance              & 270.5 & 408.2 \\
    \midrule
    Baseline preference penalty               & 318.7 & 525.3 \\
    Single-room preference penalty            & 184.1 & 516.5 \\
    Cluster ($\le 2.0$) preference penalty    & 260.4 & 663.5 \\
    Blocked-days preference penalty           & 223.3 & 386.9 \\
    \midrule
    Blocked appointments                      & 98    & 127   \\
    Blocked appointment ratio (\%)            & 23.0  & 25.5  \\
    \bottomrule
  \end{tabular}
\end{table}

These comparisons show how policy constraints change schedule quality metrics even when the
underlying appointment timing and demand are fixed. The single-room policy eliminates within-day
room switches by construction but can still trade off against preference structure. The cluster
policy reduces switching relative to baseline while preserving some room flexibility, and the
blocked-days stress test quantifies the appointment share affected by strong availability limits.

\subsection*{Admin Time, Overbooking, and Duration Uncertainty}

\textbf{Policy d (Admin time for extenuating circumstances).} We use the same administrative and lunch blocks as in the main implementation (morning admin 9:00--9:30 Mon--Thu and 8:00--8:30 Fri, noon admin, lunch closure, afternoon admin). First, we count how many appointments in the baseline schedule have any 15-minute slot overlapping those blocks (should be zero if the optimizer respected blocked time). Second, for each provider-day we count scheduled appointments; we label a provider-day as \emph{congested} if its count is at least the median across all provider-days and at least five appointments. We then report a \emph{potential} admin-time capacity figure: congested provider-days $\times$ four 15-minute slots per day, interpreted as rough headroom if morning admin were opened for overflow under stress. This quantifies where extenuating use of admin time would matter most, without actually moving visits into those intervals.

\textbf{Policy e (Overbooking, no-shows).} From each week's appointment file we compute the fraction of active (non-cancelled, non-deleted) rows marked as no-shows, denoted $r$. We treat the baseline schedule as having $N$ booked slots. Expected arrivals are $N(1-r)$; to fill nominal capacity in expectation, a standard rule is to scale bookings by $1/(1-r)$. We report $\lceil N/(1-r) - N\rceil$ as the recommended number of \emph{additional} slots to add, a planning-level overbook count. Room and provider feasibility of those extra slots is not re-checked here; the metric isolates the magnitude implied by historical no-show behavior.

\textbf{Policy f (Duration uncertainty).} For each appointment in the baseline schedule we extend its end time by a buffer: one extra 15-minute slot (+15 min), two slots (+30 min), or a 20\% increase in slot length (rounded up). Within each \emph{serial chain} (refined provider-day block the same units used in column generation), we sort visits by start time and count \emph{pairwise overlaps}: consecutive visits in that chain where the buffered end of the earlier visit is still after the start of the next. That count measures how often longer-than-nominal exams would force the same provider's sequence to overlap itself. The ratio of overlaps to the number of consecutive pairs in all chains summarizes schedule fragility under duration variability.

Table~\ref{tab:policy_def} summarizes the results. Admin time offers 156--160 potential overflow slots across congested provider-days; baseline schedules do not use admin blocks. No-show rates of 7.3\% and 7.8\% imply recommended overbooks of 34 and 43 additional slots for Weeks 1 and 2. Duration buffers of +15 or +30 minutes yield overlap ratios of roughly 62--75\% within serial chains, so the timing layout is sensitive to variability and would benefit from built-in slack or resequencing in a fuller stochastic model.

\begin{table}[h]
  \centering
  \caption{Policy validation (d, e, f): admin time, overbooking, duration uncertainty.}
  \label{tab:policy_def}
  \begin{tabular}{lrr}
    \toprule
    Metric & Week 1 & Week 2 \\
    \midrule
    \multicolumn{3}{l}{\textit{Policy d: Admin time}} \\
    Congested provider-days                    & 39    & 40    \\
    Admin capacity slots (potential)           & 156   & 160   \\
    \midrule
    \multicolumn{3}{l}{\textit{Policy e: Overbooking}} \\
    No-show rate (\%)                          & 7.3   & 7.8   \\
    Recommended additional slots               & 34    & 43    \\
    \midrule
    \multicolumn{3}{l}{\textit{Policy f: Duration uncertainty}} \\
    Overlaps with +15 min buffer               & 206   & 251   \\
    Overlap ratio (+15 min, \%)                & 61.9  & 66.6  \\
    Overlaps with +30 min buffer               & 238   & 281   \\
    Overlap ratio (+30 min, \%)                & 71.5  & 74.5  \\
    \bottomrule
  \end{tabular}
\end{table}
...
Across policy validation scenarios, the comparative trends are consistent with operational
intuition: tighter room-consistency policies reduce switch distance, while stronger availability
restrictions increase blocked-demand pressure. Because these analyses are computed on fixed
baseline schedules, they serve as a transparent validation layer for policy sensitivity without
confounding from rerun-to-rerun optimization variance.
